...
void PresetPipelineExecutor::startUdpUnicast()
{
    ...
    bool sync = false;
    if (m_ui->comboBox_9->currentText() == "Aktif") {
        sync = true;
    }
    ...
    std::string finalPipeline = GstPipelineBuilder::udpUnicastSender(sourcePipe, ip, port, bitrate, payload, sync);
...
void PresetPipelineExecutor::startUdpMulticast()
{
    ...
    if (m_ui->comboBox_10->currentText() == "Aktif") {
        syncState = true;
    }
    ...
    // 3. UDP Sender Pipeline Oluştur
    std::string finalPipeline = GstPipelineBuilder::udpMulticastSender(sourcePipe, ip, port, bitrate, payload, syncState);
